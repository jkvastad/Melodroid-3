...
table chords            unique chords per k-tet under transposition (necklaces), filtered by note count; each lists the containing LCM-family placements (--lcm-only keeps just the chords that are a subset of ≥1 available family, bounded by --max-lcm)
