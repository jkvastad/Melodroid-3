...
table progression       for any set of --chord-keys, every major/minor/dim triad it may progress to under the perception walk; default follows the strict rules (composition, 4↔3 isomorphism, bidirectional 15↔24 adjacency), --stable uses the narrower one-directional stable-superset walk (12-tet only; supersets derived via chord-melody --drop-renormalized-subsets --drop-collapsed --stable-15)
graph lcm-families      Mermaid relation graph (--mode full|collapsed; full mode labels each renormalized-subset edge with every base at which the smaller family embeds; --cluster-tolerance c adds red-dashed clustering-tolerant edges where renormalized images snap to the nearest good fraction within bin radius c — c≥1/161 surfaces exactly 18→24 via 16/9)
