...
table chord-melody      matrix of maximal-LCM placements containing a chord vs each key (maximal = not a literal subset of another allowed family; renormalized subsets kept as own rows unless --drop-renormalized-subsets, but isomorphic families always collapse to the lowest-LCM representative)
...
table progression       for a major/minor/dim --chord-keys triad, every chord it may progress to under the perception walk; both rules by default, --supersets / --adjacency restrict to one (12-tet only; perception-table data ported from website/src/lib/perceptionTables.ts)
