...
Combine with `--mode difference` to isolate the residual — the parent's superposition minus the subset's superposition. Only the difference curve is drawn (in red), with the subset's dashed period markers retained for spatial reference. This makes visible what the parent family contributes *beyond* what the subset already accounts for: where the subset captures the bulk of the pattern, the residual is small; where the parent adds genuinely new structure, the residual swings wide. Requires `--subset-lcm`; filename is `output/plots/lcm-family-{lcm}-difference-sub{K}.png`.