...
The `Min k` sequence drops in non-uniform steps (e.g. `34 → 31 → 19 → 12` with the defaults). Each drop happens at an exact bin radius `c_k = |2^(n/k) − g_k*| / g_k*` — the worst-case multiplicative distance from any good fraction to its nearest k-tet key — and is "caused" by one specific good fraction `g_k*` (the argmax of that distance). Run `dotnet run -- table ktet-cutoffs` to list, for each `k` from 1 to `--max-k`, the cutoff `c_k`, the limiting fraction `g_k*`, and the nearest k-tet key (index `n` and ratio `2^(n/k)`). Green rows mark **active** k's — those where `c_k` strictly improves over every smaller k. These are exactly the k's that ever appear as `Min k` in `ktet-min-keys`, and the green-row sequence of fractions answers "which good fraction is responsible for each transition." Options: `--max-size` (default 24), `--max-prime` (default 5), `--max-k` (default 50; this command enumerates every k rather than searching, so a smaller cap is appropriate). With default settings this notably produces...